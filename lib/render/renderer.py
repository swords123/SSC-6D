...
import numpy as np
from .utils.decoder_utils import decode_sdf, decode_sdf_gradient
from .utils.render_utils import depth2normal
import copy
import time
...
class SDFRenderer(object):
    def __init__(self, decoder, march_step=50, buffer_size=5, ray_marching_ratio=1.5, max_sample_dist=0.2, radius=1.0, threshold=5e-5, scale_list=[4, 2, 1], march_step_list=[3, 3, -1], use_gpu=True, is_eval=True):
    ...
    def set_intrinsic(self, intrinsic, img_hw=None, transform_matrix=None):
    ...
    def get_intersections_with_unit_spheres(self, cam_pos, cam_rays):
        ...
        with torch.no_grad():
            ...
            valid_mask = (dist <= self.radius)#.to(torch.uint8)
            maxbound_marching_zdepth = self.get_maxbound_zdepth_from_dist(dist) # (H*W)

            cam_pos_dist = torch.sqrt((cam_pos ** 2).sum())
            if torch.nonzero((cam_pos_dist < self.radius).unsqueeze(0), as_tuple=False).shape[0] != 0:
    ...
    def copy_index(self, inputs, mask, src):
        ...
        index = torch.nonzero(mask, as_tuple=False).reshape(-1).long()
    ...
    def ray_marching_recursive(self, cam_pos, cam_rays, init_zdepth, valid_mask, latent, march_step=None, stop_threshold=None, clamp_dist=0.1, no_grad=False, use_transform=True, use_first_query_check=True):
        ...
        for i in range(march_step):
            ...
            if torch.nonzero(unfinished_mask, as_tuple=False).shape[0] == 0:
    ...
    def get_downscaled_grid_map(self, grid_map, scale=2.0):
        ...
        new_grid_map = self.get_meshgrid((new_h, new_w)).to(grid_map.get_device())
    ...
    def maxpool_valid_mask_with_index(self, valid_mask, index_map):
        ...
        with torch.no_grad():
            valid_mask = valid_mask.to(torch.uint8)
            new_valid_mask, _ = scatter_max(valid_mask, index_map)
            new_valid_mask = new_valid_mask.to(torch.bool)
    ...
    def render_depth(self, latent, R, T, clamp_dist=0.1, sample_index_type='min_abs', profile=False, no_grad=False, no_grad_depth=False, no_grad_mask=False, no_grad_camera=False, ray_marching_type='recursive', use_transform=True):
        ...
        min_sdf_sample, points_for_min_sdf = self.get_min_sdf_sample_and_points(sdf_list, points_list, latent, index_type='min_abs', clamp_dist=clamp_dist, profile=profile, no_grad=no_grad_mask)

        # #================visulation point list for temp===================================
        # print('----------------')

        # print(points_for_min_sdf.shape)

        # pcd_p = o3d.geometry.PointCloud()
        # pcd_p.points = o3d.utility.Vector3dVector(points_for_min_sdf.detach().cpu().numpy())
        # o3d.visualization.draw_geometries([pcd_p])

        # # for i in range(points_list.shape[0]):
        # #     print(points_list[i].shape)
        # #     pcd_p = o3d.geometry.PointCloud()
        # #     pcd_p.points = o3d.utility.Vector3dVector(points_list[i].detach().cpu().numpy())
        # #     o3d.visualization.draw_geometries([pcd_p])
        # print('------------------')
        # #===================================================        
        ...
        #=============================
        # plt.figure()
        # plt.imshow(valid_mask.detach().cpu().numpy().reshape(128,128))
        # plt.show()
        #=============================
        ...
        valid_mask[valid_mask] = valid_mask_render#.to(torch.uint8)

        #=================================
        # print(points_for_min_sdf.shape)
        # print(valid_mask_render.shape)
        # print(valid_mask_render)
        #=================================

        points_for_min_sdf = points_for_min_sdf[valid_mask_render]
        ...
        if no_grad_depth:
            Zdepth = Zdepth.detach()
        return Zdepth, valid_mask, min_sdf_sample_new, points_for_min_sdf # (H*W), (H*W), (H*W)
    ...
    def render(self, latent, R, T, clamp_dist=0.1, sample_index_type='min_abs', profile=False, no_grad=False, no_grad_depth=False, no_grad_mask=False, no_grad_camera=False, normalize_normal=True, use_transform=True, ray_marching_type='pyramid_recursive', num_forward_sampling=0):
        '''
        ...
        # render depth
        Zdepth, valid_mask, min_abs_query, points_min_sdf = self.render_depth(latent, R, T, clamp_dist=clamp_dist, sample_index_type=sample_index_type, profile=profile, no_grad=no_grad, no_grad_depth=no_grad_depth, no_grad_mask=no_grad_mask, no_grad_camera=no_grad_camera, ray_marching_type=ray_marching_type, use_transform=use_transform) # (H*W), (H*W), (H*W)
        ...
        return depth, binary_mask, min_abs_query, points_min_sdf
