...
def compute_loss_mask(min_sdf_sample, valid_mask, valid_mask_gt, threshold=5e-5, name=['mask_output', 'mask_gt', 'loss_mask_gt', 'loss_mask_out'], handle_first_query_corner_case=False):
    '''
    compute mask loss
    Input:
    ...
    loss_mask_out = loss_mask_out.mean()
    return loss_mask_gt, loss_mask_out

def compute_loss_depth(depth_output, valid_mask, depth_gt, valid_mask_gt):
    '''
...
def compute_loss_normal(normal_output, valid_mask, normal_gt, valid_mask_gt):
    '''
